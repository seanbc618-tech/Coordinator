"""Phase 9 E2E tests for delivery RPCs and slash routing."""
...
class DeliveryRpcTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_project_deliver_rpc_exists(self) -> None:
        resp = self.methods.handle(
            self.conn,
            _request("project.deliver", self.project_id, task_id=self.task_id),
        )
        self.assertTrue(resp.ok, resp.error)
        self.assertIn("allowed", resp.result)

    def test_project_prs_rpc_is_project_scoped(self) -> None:
        resp = self.methods.handle(
            self.conn,
            _request("project.prs", self.project_id),
        )
        self.assertTrue(resp.ok, resp.error)
        self.assertEqual(resp.result.get("project_id"), self.project_id)

    def test_project_merge_policy_rpc_reports_repo_policy(self) -> None:
        resp = self.methods.handle(
            self.conn,
            _request("project.merge_policy", self.project_id),
        )
        self.assertTrue(resp.ok, resp.error)
        repos = resp.result.get("repos") or []
        self.assertTrue(repos)


class Phase9SlashRoutingTests(unittest.TestCase):
    def setUp(self) -> None:
        self.tmp = Path(tempfile.mkdtemp())
        self.home = self.tmp / "home"
        self.home.mkdir()
    ...
    def test_deliver_slash_maps_to_project_deliver(self) -> None:
        _run_cli_with_home(
            self.home, "--mode", "rpc", "-p", "/deliver task-1", cwd=self.repo
        )
        methods = [m for m, _ in self.server.drain_requests()]
        self.assertIn("project.deliver", methods)

    def test_prs_slash_maps_to_project_prs(self) -> None:
        _run_cli_with_home(self.home, "--mode", "rpc", "-p", "/prs", cwd=self.repo)
        methods = [m for m, _ in self.server.drain_requests()]
        self.assertIn("project.prs", methods)

    def test_ci_slash_maps_to_project_ci(self) -> None:
        _run_cli_with_home(
            self.home, "--mode", "rpc", "-p", "/ci task-1", cwd=self.repo
        )
        methods = [m for m, _ in self.server.drain_requests()]
        self.assertIn("project.ci", methods)

    def test_delivery_slash_maps_to_project_delivery(self) -> None:
        _run_cli_with_home(
            self.home, "--mode", "rpc", "-p", "/delivery task-1", cwd=self.repo
        )
        methods = [m for m, _ in self.server.drain_requests()]
        self.assertIn("project.delivery", methods)

    def test_merge_policy_slash_maps_to_project_merge_policy(self) -> None:
        _run_cli_with_home(
            self.home, "--mode", "rpc", "-p", "/merge-policy", cwd=self.repo
        )
        methods = [m for m, _ in self.server.drain_requests()]
        self.assertIn("project.merge_policy", methods)
